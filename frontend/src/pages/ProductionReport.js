...
function ProductionReport() {
  ...
  const handleConsumir = async () => {
    await dispatch(consumeProduction());
  };

  const suggestions = relatorio?.suggestions || [];
  const totalValue =
    relatorio?.totalValue ??
    suggestions.reduce(
      (sum, sugestao) => sum + (parseFloat(sugestao.totalValue) || 0),
      0,
    );
      ...
      <div className="card">
        ...
        >
          ...
          <div style={{ display: "flex", gap: "0.5rem" }}>
            ...
            <button onClick={handleConsumir} className="btn btn-danger">
              Produzir e Baixar Estoque
            </button>
          </div>
        </div>
        ...
        {suggestions.length > 0 ? (
              ...
              <tbody>
                {suggestions.map((sugestao) => (
                  <tr key={sugestao.productId}>
                    ...
                    <td>
                      R${" "}
                      {parseFloat(sugestao.unitProfit || 0)
                        .toFixed(2)
                        .replace(".", ",")}
                    </td>
                    ...
                    <td
                      style={{
                        color: (sugestao.profit || 0) >= 0 ? "green" : "red",
                        fontWeight: "bold",
                      }}
                    >
                      R${" "}
                    ...
                    <td
                      style={{
                        color:
                          (sugestao.profitMargin || 0) >= 0 ? "green" : "red",
                        fontWeight: "bold",
                      }}
                    >
          ...
          >
            <p>
              Nenhum produto pode ser produzido com os níveis de estoque atuais.
            </p>
        ...
        >
          ...
          <ul style={{ marginLeft: "1.5rem", marginTop: "0.5rem" }}>
            ...
            <li>
              Matérias-primas são alocadas de forma otimizada para maximizar o
